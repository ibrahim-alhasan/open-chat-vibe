...
import { MessageSquare, Image, ChevronRight, Search, RefreshCw, Ban, ShieldOff } from "lucide-react";
...
const AdminPanel = ({ profilesMap }: AdminPanelProps) => {
  ...
  const fetchTabData = async (activeTab: Tab) => {
    setLoading(true);
    try {
      if (activeTab === "conversations") {
        const allDms = await fetchAllRows("direct_messages", supabase.from("direct_messages").select("*").order("created_at", { ascending: false }));
        ...
        for (const dm of allDms) {
          ...
          if (!convMap.has(key)) {
            ...
            convMap.set(key, {
              ...
              usernameA: profileA.username, usernameB: profileB.username,
              avatarA: profileA.avatar_url, avatarB: profileB.avatar_url,
              lastMessage: dm.content, lastTime: dm.created_at, messageCount: 1,
            });
          } else {
        ...
        const allImages = await fetchAllRows("direct_messages", supabase.from("direct_messages").select("*").not("image_url", "is", null).order("created_at", { ascending: false }));
        setImages(allImages as DmImage[]);
  ...
  // Re-fetch when loadedTabs changes and current tab is missing
  useEffect(() => {
    if (!loadedTabs.has(tab)) fetchTabData(tab);
  }, [loadedTabs]);
  ...
  const openConversation = async (userA: string, userB: string) => {
    ...
    const allMessages = await fetchAllRows("direct_messages",
      supabase.from("direct_messages").select("*")
        .or(`and(sender_user_id.eq.${userA},receiver_user_id.eq.${userB}),and(sender_user_id.eq.${userB},receiver_user_id.eq.${userA})`)
  ...
  const refreshConversation = async () => {
    ...
    const allMessages = await fetchAllRows("direct_messages",
      supabase.from("direct_messages").select("*")
        .or(`and(sender_user_id.eq.${selectedConv.userA},receiver_user_id.eq.${selectedConv.userB}),and(sender_user_id.eq.${selectedConv.userB},receiver_user_id.eq.${selectedConv.userA})`)
        .order("created_at", { ascending: true })
    );
    setConvMessages(allMessages);
    setRefreshing(false);
    ...
    return (
      <div className="flex flex-col h-full" style={{ background: "hsl(var(--chat-bg, var(--background)))" }}>
        <div className="flex items-center gap-3 px-4 py-3 flex-shrink-0" style={{ background: "hsl(var(--chat-header, var(--card)))", borderBottom: "1px solid hsl(var(--border))" }}>
          ...
          <button onClick={refreshConversation} disabled={refreshing} className="p-2 rounded-full hover:opacity-70 transition-all active:scale-90" style={{ color: "hsl(var(--primary))" }}>
              ...
              return (
                <div key={msg.id} className={`flex gap-2 animate-fade-in ${isUserA ? "flex-row" : "flex-row-reverse"}`}>
                  ...
                  <div className={`max-w-[75%] flex flex-col ${isUserA ? "items-start" : "items-end"}`}>
                    <span className="text-[11px] font-semibold mb-0.5 px-1" style={{ color: senderColor }}>{senderProfile.username}</span>
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="px-3 py-2 flex items-center gap-2">
        <div className="flex-1 flex items-center gap-2 px-3 py-2 rounded-full" style={{ background: "hsl(var(--secondary))" }}>
        ...
        <button onClick={handleRefreshTab} className="p-2 rounded-full hover:opacity-70 transition-all active:scale-90" style={{ color: "hsl(var(--primary))" }}>
          ...
          filteredConversations.length === 0 ? (
            ...
            <div className="space-y-1.5">
              {filteredConversations.map((conv, i) => (
                <button key={i} onClick={() => openConversation(conv.participantA, conv.participantB)}
                  ...
                  <div className="flex-1 min-w-0 text-right">
                    <div className="flex items-center justify-between">
                      <span className="text-[13px] font-semibold" style={{ color: "hsl(var(--foreground))" }}>{conv.usernameA} ↔ {conv.usernameB}</span>
                    ...
                    <div className="flex items-center justify-between mt-0.5">
                      ...
                      <span className="text-[10px] px-1.5 py-0.5 rounded-full" style={{ background: "hsl(var(--primary) / 0.15)", color: "hsl(var(--primary))" }}>{conv.messageCount} رسالة</span>
                    </div>
                  </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {filteredImages.map((img) => (
                <div key={img.id} className="rounded-xl overflow-hidden" style={{ background: "hsl(var(--secondary))" }}>
                  <img src={img.image_url} alt={img.image_name || ""} className="w-full h-32 object-cover cursor-pointer hover:opacity-90 transition-opacity" onClick={() => setLightboxImg(img.image_url)} />

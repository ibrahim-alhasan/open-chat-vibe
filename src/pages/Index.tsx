...
const Index = () => {
  ...
  const wasPublicChatVisibleRef = useRef(false);
  ...
  // just like opening an Android conversation screen.
  useEffect(() => {
    const isPublicChatVisible = location.pathname === "/";
    if (!isPublicChatVisible) {
      wasPublicChatVisibleRef.current = false;
      return;
    }
    if (wasPublicChatVisibleRef.current || loading || messages.length === 0) return;

    let cancelled = false;
    const frame = requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        if (!cancelled) forceScrollToBottom();
      });
    });
    wasPublicChatVisibleRef.current = true;

    return () => {
      cancelled = true;
      cancelAnimationFrame(frame);
    };
  }, [forceScrollToBottom, loading, location.pathname, messages.length]);
      ...
      <header className="chat-app-header flex-shrink-0 px-3 py-2.5 flex items-center justify-between overflow-hidden z-20" style={{ background: "hsl(var(--chat-header))", borderBottom: "1px solid hsl(var(--border))" }}>
        ...
        <div className="chat-header-actions">
          <MaterialIconButton
            ...
            ariaLabel="المزيد من الخيارات"
            ariaControls={headerMenuAnchor ? "main-header-menu" : undefined}
            ariaHasPopup="menu"
            ariaExpanded={Boolean(headerMenuAnchor)}
            onClick={(event) => setHeaderMenuAnchor(event.currentTarget)}
            icon={<MoreVertIcon />}
            badge={!isGuest && unreadDMs > 0 ? (unreadDMs > 99 ? "99+" : unreadDMs) : undefined}
          />
          <Menu
            id="main-header-menu"
      ...
      >
        ...
        {(showScrollButton || hasNewMessages) && (
          <div className="fixed bottom-24 right-4 flex flex-col items-end gap-2 z-10">
            ...
            {showScrollButton && !hasNewMessages && (
              <MaterialIconButton
                ariaLabel="الانتقال إلى آخر رسالة"
        ...
        <div className="chat-app-composer flex-shrink-0 px-3 pb-3 pt-1.5 fixed bottom-0 left-0 right-0 z-20" style={{ background: "hsl(var(--chat-bg))" }}>
          ...
          <div className="flex items-end gap-2">
            ...
            <MaterialIconButton
              ...
              className={`chat-send-button ${(input.trim() || selectedFile) && !sending ? "is-ready" : ""}`}
              onClick={handleSend}
